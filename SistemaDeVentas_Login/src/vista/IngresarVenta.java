...
public class IngresarVenta extends javax.swing.JFrame {
    ...
    public IngresarVenta() {
        ...
        jTable1_Venta.setModel(tableModel);
    }
    ...
    private void jComboBox1_tipo1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jComboBox1_tipo1ActionPerformed
        if (jComboBox1_tipo1.getSelectedItem().equals("Boleta")) {
            txt_rucVenta.setEnabled(false);
            txt_rucVenta.setText("");
        } else {
            txt_rucVenta.setEnabled(true);
            txt_rucVenta.setText("");
        }
    ...
    private void jButton1_AgregarVentaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1_AgregarVentaActionPerformed
        if(txt_cantidad.getText().equals("") || Integer.parseInt(txt_cantidad.getText()) == 0){
            JOptionPane.showMessageDialog(this, "Favor de ingresar la cantidad", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        String valorProducto = (String) jComboBox1_ProductosVenta.getSelectedItem();
        ...
        for (Productos producto : ProductoManager.getProductos()) {
            if (producto.getNombre().equals(valorProducto)){
                ...
                tableModel.addRow(new Object[]{
                    producto.getCodigo(), producto.getNombre(), producto.getPrecio(), cantidad, totalProducto
                });
                this.calcularTotal();
                return;
            }
    ...
    private void guardarVenta() {
        String nombreCliente = txt_nombreVenta.getText();
        String codigoDocumento = txt_documentoVenta.getText();
        String tipoVenta = (String) jComboBox1_tipo1.getSelectedItem();
        String ruc = txt_rucVenta.getText();
        String telefono = txt_TelefonoVenta.getText();
        String fechaRegistro = txt_Fecha.getText();
        double costoTotal = Double.parseDouble(txt_pagoVenta.getText());
        ...
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            productos.add(new ProductoVenta(codigoProducto, cantidad));
        }
        
        int codigoVenta = VentaManager.getVenta().size();

        Venta venta = new Venta(codigoVenta, nombreCliente, codigoDocumento, tipoVenta, ruc, telefono, productos, fechaRegistro, costoTotal);
        VentaManager.getVenta().add(venta);
    ...
    private void limpiarFormulario() {
        txt_nombreVenta.setText("");
        txt_documentoVenta.setText("");
        txt_rucVenta.setText("");
        txt_TelefonoVenta.setText("");
        txt_cantidad.setText("");
        txt_pagoVenta.setText("");
